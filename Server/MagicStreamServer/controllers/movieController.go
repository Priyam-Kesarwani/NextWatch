...
func GetMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if movieCollection == nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Movies collection is not available"})
			return
		}
		...
		// Ensure an empty array (not null) is returned when there are no movies
		if movies == nil {
			movies = []models.Movie{}
		}
